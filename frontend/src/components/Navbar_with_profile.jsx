      ...
      <div className="max-w-7xl mx-auto px-8">
        <div className="flex justify-between items-center h-24">
          ...
          <div className="flex items-center space-x-8">
            ...
            <Link to="/community" className={`${isDark ? 'text-gray-300 hover:text-violet-400' : 'text-gray-700 hover:text-orange-600'} transition-all duration-300 font-medium relative group`}>
              Community
              <div className={`absolute -bottom-1 left-0 w-0 h-0.5 ${isDark ? 'bg-violet-400' : 'bg-orange-500'} group-hover:w-full transition-all duration-300`}></div>
            </Link>
            
            <Link to="/activities" className={`${isDark ? 'text-gray-300 hover:text-violet-400' : 'text-gray-700 hover:text-orange-600'} transition-all duration-300 font-medium relative group`}>
              Activities
              <div className={`absolute -bottom-1 left-0 w-0 h-0.5 ${isDark ? 'bg-violet-400' : 'bg-orange-500'} group-hover:w-full transition-all duration-300`}></div>
            </Link>
            ...
            <div className={`flex items-center space-x-4 pl-8 border-l ${isDark ? 'border-gray-700/50' : 'border-gray-300/50'}`}>
              <div 
                className={`w-12 h-12 ${isDark ? 'logo-gradient' : 'logo-gradient-light'} rounded-2xl flex items-center justify-center text-lg font-bold shadow-lg cursor-pointer hover:scale-105 transition-transform`}
              ...
              <div className="flex flex-col">
                <button 
                  onClick={handleProfileClick}
                  className={`font-semibold hover:underline text-left ${isDark ? 'text-white hover:text-violet-400' : 'text-gray-900 hover:text-orange-600'} transition-colors`}
                >
                  {user.first_name}
                </button>
                <button 
