...
function NewTripPlanner({ user, isDark }) {
  ...
  const [itineraryData, setItineraryData] = useState({ totalBudget: 0, sections: [] })
  ...
  const handleItineraryChange = (totalBudget, sections) => {
    setItineraryData({ totalBudget, sections })
  }
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
      <div className="max-w-6xl mx-auto p-6 pt-8">
        ...
        {/* Suggestions Section */}
        <div className={`border rounded-2xl p-6 mb-8 ${isDark ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-white'}`}>
        ...
        <ItinerarySection onChange={handleItineraryChange} isDark={isDark} />
        ...
        <div className="mt-8">
          ...
          >
            {loading ? 'Creating Trip...' : `Create Trip (₹${itineraryData.totalBudget.toLocaleString()} estimated)`}
          </button>
        </div>
      </div>
    </div>
